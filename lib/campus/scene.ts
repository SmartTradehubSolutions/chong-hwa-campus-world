...
export function createCampus(host:HTMLDivElement,options:Options){
 ...
 function animate(now:number){if(disposed)return;raf=requestAnimationFrame(animate);const dt=Math.min((now-last)/1000,.05);last=now;if(overlayPaused)return;
