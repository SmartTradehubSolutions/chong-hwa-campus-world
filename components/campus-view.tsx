...
type Props={mode:ViewMode;selected:string|null;labels:boolean;orbit:boolean;avatar:'boy'|'girl';paused:boolean;suspended?:boolean;onSelect:(id:string)=>void;onWalkState:(state:WalkState)=>void;onWalkInteract:(id:string)=>void;onReady:()=>void;onError:(message:string)=>void;apiRef:React.RefObject<CampusApi|null>};
export default function CampusView(props:Props){
 const{mode,selected,labels,orbit,avatar,paused,suspended=false,apiRef}=props;
 ...
 useEffect(()=>{let cancelled=false;import('@/lib/campus/scene').then(({createCampus})=>{if(cancelled||!host.current)return;try{
  ...
  const p=latest.current;if(p.mode!=='world')scene.setMode(p.mode);scene.setSelected(p.selected);scene.setLabels(p.labels);scene.setOrbit(p.orbit);scene.walk.setAvatar(p.avatar);scene.walk.setPaused(p.paused);scene.setOverlayPaused(p.suspended??false);
 ...
 useEffect(()=>apiRef.current?.setOverlayPaused(suspended),[suspended,apiRef]);
